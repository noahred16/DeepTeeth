...
def evaluate(model, dataloader, criterion, device, visualizer, desc="Eval", capture_embeddings=False):
    ...
    visualizer._capture_embeddings = capture_embeddings
    ...
    with torch.no_grad():
        ...
        for images, labels in progress_bar:
            ...
            visualizer._current_labels = labels
    ...
    visualizer._current_labels = None
    visualizer._capture_embeddings = False
...
def main():
    ...
    visualizer.embeddings.clear()
    visualizer.labels.clear()
    visualizer._capture_embeddings = True
    ...
    if visualizer.embeddings and visualizer.labels:
        test_features = torch.cat(visualizer.embeddings)
        test_labels = torch.cat(visualizer.labels)
        print(f"[INFO] Logging {test_features.shape[0]} test embeddings to TensorBoard")
        metadata = [CLASS_NAMES[int(label)] for label in test_labels]
        visualizer.writer.add_embedding(
            test_features,
            metadata=metadata,
